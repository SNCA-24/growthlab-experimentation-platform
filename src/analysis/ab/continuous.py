...
def analyze_continuous_metric(
    ...
    *,
    ...
    if metric.metric_type != MetricType.continuous:
        raise TypeError(f"metric '{metric.metric_name}' is not continuous")
    ...
    analysis[metric.metric_column] = y.reset_index(drop=True)
    if covariate is not None and metric.covariate_column is not None:
        analysis[metric.covariate_column] = covariate.reset_index(drop=True)
